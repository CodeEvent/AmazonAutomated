...
This design system is adapted here to power a stay/holiday booking marketplace (booking.com-style catalog and checkout flow) rather than Airbnb's own product surface. Tokens, spacing, radii and component names are reused as-is from the source analysis; only the domain content (search verticals, listing content) changes.
